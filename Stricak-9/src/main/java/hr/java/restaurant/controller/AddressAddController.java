...
public class AddressAddController implements AddController {
    ...
    public void add() {
        ...
        if (!errorMessage.isEmpty()) {
            AlertDialog.showErrorDialog("Address save error", errorMessage);
            logger.error("Address save error: {}", errorMessage);
            return;
        }

        Address address = new Address.Builder(addressRepository.findNextId())
                .setStreet(street)
